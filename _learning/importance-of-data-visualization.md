...
- In the field of Machine Learning, data visualization is not just making fancy graphics for reports.
- It is used extensively in day-to-day work for all phases of a project.
...
- It is much more convenient for us, humans, to grasp the main points this way than by reading many lines of raw data.
- It is amazing how much insight can be gained from seemingly simple charts created with available visualization tools.
...
- Its math is also impressive (we will not delve into it here, but, if you feel brave, here is the original article by Laurens van der Maaten and Geoffrey Hinton from JMLR).
...
1. High computational complexity. The implementation in scikit-learn is unlikely to be feasible in a real task. If you have a large number of samples, you should try Multicore-TSNE instead.
2. The plot can change a great deal depending on the random seed, which complicates interpretation. In general, you shouldn’t make any far-reaching conclusions based on such graphs because it can equate to plain guessing. Of course, some findings in t-SNE pictures can inspire an idea and be confirmed through more thorough research down the line, but that does not happen very often.
3. Occasionally, using t-SNE, you can get a really good intuition for the data. The following is a good paper that shows an example of this for handwritten digits: Visualizing MNIST.